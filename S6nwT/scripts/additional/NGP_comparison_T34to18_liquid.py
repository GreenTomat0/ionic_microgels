...
def draw_NGP():
	...
	for i, (t, n) in enumerate(zip(tau_sp, NGP_sp)):
		...
		plt.plot(t, n, ls='none', marker='^', ms=10, markerfacecolor="None", markeredgewidth=2, label=str(time_label) + ' min') #label=f'{series}_{frame}')
...
def draw_MSD():
	...
	for s in series:
		...
		for fr in frame_range:
			...
			plt.plot(tau, MSD_micron, ls='none', marker='^', ms=10, markerfacecolor="None", markeredgewidth=1, label=str(time_label) + ' min') #label=f'{s}_{fr}')
